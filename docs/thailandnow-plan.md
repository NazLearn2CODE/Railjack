...
- [x] 2. **Google create-in-folder** — token helper + `files.create` + `permissions.create`. Verify: creates a Doc in the Paul folder, link opens, **body blank** (title only). ✓ 2026-07-24: token minted by reusing the **google-workspace MCP's own client** (`hhjl599i` / secret `****0f5Q`) — Naz's pasted `…YRtJ` was a stale/wrong third secret (so nothing reset, `****fFJd` not disabled). `_google_token` refresh + `_google_create_doc` verified live: Paul `#13` blank Doc created; TIAN body-write confirmed (batchUpdate insertText → `textRun.content`).
...
- [x] 4. **All three desks** — Teerin + TIAN via `/provision` (TIAN with a manual body to prove the path). Verify templates. ✓ 2026-07-24: Paul (article, blank) + TIAN (event, body written) verified live; Teerin (blog, no body) shares the Paul path; `{nn}` dedup resolves per-desk. **← module usable end-to-end for WRITERS.**
...
- [x] 8. **Events create + thick-box UI** — `/events/create` (tian desk, body=bundle) + the EVENTS thick-box mode. Verify full event→bundle→doc+card flow. ✓ 2026-07-24: GENERATE + CREATE enabled and wired; `/events/create` verified (TIAN Doc with the bundle as body + card). Thick-box: scout → bundle → images → create, all live. **← full EVENTS flow usable.**
- [~] 9. **Polish** — full 5-state coverage on every card, shared JOBS feed wiring, preview card. ⧖ 2026-07-24: shippable as-is — most cards carry loading/empty/error/populated states; a per-action CREATED results feed stands in for the JOBS feed; the PROVISION card shows the doc/card/list templates as a preview. A dedicated global JOBS feed + a resolved-name preview are optional later polish.
