...
- [ ] 2. **Google create-in-folder** — token helper + `files.create` + `permissions.create`. Verify: creates a Doc in the Paul folder, link opens, **body blank** (title only).
- [ ] 3. **Trello create+attach + `{nn}` dedup scan** — `/provision` end-to-end for Paul. Verify: 12 docs + 12 cards, doc attached to each, names `[202607] [CAT] #01..#12`, re-run yields #13+.
...
- [ ] 5. **Events scout + parser** — `/events/scout` via Jina. Verify: returns real events with URLs.
- [ ] 6. **Publicity bundle** — `/events/publicize` (LLM + gem + Jina fetch). Verify: a real event URL yields the 5-part plain-text bundle, THB-first, no markdown.
- [ ] 7. **Image lookup** — `/events/images` (Jina scrape + Pexels/Pixabay). Verify thumbnails render.
- [ ] 8. **Events create + thick-box UI** — `/events/create` (tian desk, body=bundle) + the EVENTS thick-box mode. Verify full event→bundle→doc+card flow. **← full EVENTS flow usable after this slice.**
