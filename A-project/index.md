...
- **Next — follow-on queue:** the dedicated **multi-lane team view is YAGNI** until delegations run concurrently — `Team.delegate()` awaits one `worker.run()` per tool call (sequential hub-and-spoke), so there is nothing to watch side-by-side; its trigger is a `delegate_many` fan-out primitive, which doesn't exist. Other candidate: native Anthropic-API verification (approval gate proven on z.ai only). *Deferred (YAGNI, w/ triggers):* concurrent fan-out delegation; per-role `allowed_tools` editing (API ships name+system_prompt only today); hierarchical/decentralized topologies; shared team token-budget pool; per-worker autonomy levels; **Channel** (second microkernel trait — single impl today, the WS gateway).
