...
| M6.3 | Autostart install + browser-tab rename | ✅ 2026-07-18 (host-built directly, Naz-requested. **(a)** Autostart: installed `assets/railjack.service` as a systemd USER unit (`enable --now` → `active`+`enabled`, symlinked into `default.target.wants` so it survives reboot); replaced the ad-hoc `uvicorn &` a prior session left on :8700 (killed first to free the port); z.ai key wired via `EnvironmentFile=-~/.config/railjack/env` (chmod 600, mirrored from `~/.bashrc`); `.desktop` + `orbiter-dev` icon installed to `~/.local/share`. **(b)** Tab: `web/index.html` title `ORBITER · Agentic OS Console`→`RAILJACK BRIDGE`, `dist/` rebuilt, live curl confirmed served title; commit `6a601d1`. **(c)** Office instance name recorded → `Somatic` (name-only, rename deferred). **(d, follow-up commit `7ae38d9`)** Telemetry RESET now shows only for real API data — Anthropic's `/usage` is tightly rate-limited (429s intermittently), so `_fetch_usage` retains the last-good reading 10 min across transient failures instead of flapping to the JSONL estimate; frontend drops the `~`-marked estimate reset entirely. Strip reads CTX/SES/WK/RESET for real telemetry, bare CTX otherwise. 63 pytest; live strip verified `ANTHROPIC CTX/SES 80%/WK 48%/RESET 3:40`, no tilde. **(e, commit `27dbd60`)** MARKETPLACE dropdown — the SKILLS menu only scanned `~/.claude/skills/`, missing plugin skills; new `/api/catalog` `marketplace_skills` scans each installed plugin's `skills/` dir (from `plugins/installed_plugins.json`→installPath), grouped by plugin (marketing 45 first, then superpowers 14, ponytail 6, frontend-design, skill-creator = 67 total). Third TopBar `<select>` between SKILLS and MCP; native popup scrolls; insert = fully-qualified `/plugin:name ` (commit `0686658`, e.g. `/marketing-skills:marketing-plan`, `/code-review:code-review`) so plugin skills never collide with a personal skill. 65 pytest; live 5 groups/67 skills, headless 0 console errors.) |
| M6.6 | Cockpit relocation + glow polish | ✅ 2026-07-19 (GLM child via agent-x from `B-sessions/brief-m6.6-cockpit-relocation.md`; host-verified. TopBar dissolved: SKILLS/MARKETPLACE/MCP dropdowns + compact BOOTSTRAP/COMMIT&PUSH buttons (`.btn--compact`) moved into the FramePanel tab header (`CockpitControls.tsx`); 2-line `▸ RAILJACK` / `// TAWHAN` wordmark atop ModuleRail so the frame reaches the top border; tab titles glow via `.panel-title`; IFRAME/PANEL kind label removed; FfmpegPanel's duplicate inner VIDEO LAB title removed (live-only RUNNING chip stays). Canned prompts now sync/push SomaticRailjack too (BOOTSTRAP: pull both; COMMIT&PUSH: push `machine/railjack` backup only, never shared main — commit `2a98e30`). Verified: 151 pytest, tsc+vite build, oxlint, headless UI — rail/frame top aligned (8==8), glow computed, IFRAME absent, no dup title, 3 selects + 2 compact buttons in header on every tab, 0 console errors. Commit `3a82606`, backed up to `machine/railjack`.) |
