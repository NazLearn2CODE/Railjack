...
- **Next — actionable worker approvals:** today a worker's gated tool (the `coder` role's Write/Edit/Bash) blocks until `APPROVAL_TIMEOUT` (600s) → fail-closed, because workers aren't registered with the manager (the `researcher` role is read-only and already works). Make them actionable with **no new endpoint**:
  - *Backend:* `AgentSession.kind = "worker"`; `Team` takes a `register` callback (mirror the `delegate`-callback pattern) wired from `manager.register` in `main.py:create_team`; `Team.delegate()` registers each worker before `worker.run()`; `AgentSessionManager.list_sessions()` excludes `kind == "worker"` (keep the sidebar clean — workers are observed via the supervisor's lane). The existing `POST /api/sessions/{id}/approve` + `/ws/sessions/{id}` then drive a worker unchanged.
  - *Frontend:* the `worker_lane` row already captures `approval` from the forwarded `approval_needed` — render a real `ApprovalCard` (reuse the one in `Console.tsx`) that POSTs to `/api/sessions/{workerId}/approve`, cleared on resolution (invoke `frontend-design`).
  - *Test:* `FakeProvider` worker emitting `approval_needed`; assert `manager.get_session(worker_id)` returns it and `approve_tool()` resolves the pending future.
  - *Follow-on queue:* a dashboard surface for `ORBITER_MCP_SERVERS` (env-only today) → dedicated multi-lane team view → in-dashboard role editor.
  - *Deferred (YAGNI):* hierarchical/decentralized topologies; shared team token-budget pool; per-worker autonomy levels; **Channel** (second microkernel trait — single impl today, the WS gateway); native Anthropic-API verification (approval gate proven on z.ai only).
