...
- **Next — follow-on queue:** the dedicated **multi-lane team view is YAGNI** until delegations run concurrently — `Team.delegate()` awaits one `worker.run()` per tool call (sequential hub-and-spoke), so there is nothing to watch side-by-side; its trigger is a `delegate_many` fan-out primitive, which doesn't exist. **Native Anthropic-API verification** was investigated this session → **deferred w/ recipe** (`[[2026-07-02-native-anthropic-verification-deferred]]`): backend selection is env-only, so it's a pure key swap — but no `sk-ant-` key exists in env/`.env` (only the z.ai token); the ADR holds the exact launch + drive recipe to finish it in minutes once a key is supplied. *Deferred (YAGNI, w/ triggers):* concurrent fan-out delegation; per-role `allowed_tools` editing (API ships name+system_prompt only today); hierarchical/decentralized topologies; shared team token-budget pool; per-worker autonomy levels; **Channel** (second microkernel trait — single impl today, the WS gateway).
