...
- **Next stage — Channel + orchestration** (per `[[architecture]]`: dashboard/gateway + full 4-layer security + Provider trait done): (1) **Channel** `Protocol` (messaging) — abstract the WS-gateway transport (inbound prompts/approvals + outbound event bus) once a second channel materializes; then (2) **orchestration** — MCP host/client/server + 2DOT topologies (guide Phase 2). Native Anthropic-API verification remains a lower-priority cross-check (approval gate proven on z.ai only).
