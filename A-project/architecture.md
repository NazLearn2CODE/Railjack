...
| 2DOT topologies | **Centralized DONE (concurrent fan-out)** — `Team`/`WorkerRole` (`app/core/orchestrator.py`): supervisor fans out via an in-process `delegate_many` MCP tool (`asyncio.gather` over N workers); a worker IS an `AgentSession` (full scheduler + security apply). Sequential `delegate` superseded; Hierarchical/decentralized deferred |
