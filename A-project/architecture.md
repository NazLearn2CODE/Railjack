...
│  L1 workspace boundary · L2 shell policy ·              │
│  L3 Landlock→Bubblewrap→Docker · L4 HMAC tool receipts  │
└─────────────────────────────────────────────────────────┘
```
...
| Tool receipts | `hmac.new(secret, payload, sha256)` per tool call → append-only audit log |
